...
stopifnot(identical(colnames(X), hvg))
...
#-----------------------
#  make ctpnet from full 
#-----------------------
X_full <- t(as.matrix(GetAssayData(seu_prep, assay="RNA", layer="data")))
stopifnot(identical(rownames(X_full), colnames(seu_prep)))
rna_train_full <- X_full[train_cells, , drop=FALSE]
rna_test_full  <- X_full[test_cells,  , drop=FALSE]

write.csv(rna_train_full, file.path(out_ctp, "rna_train_full.csv"))
write.csv(rna_test_full,  file.path(out_ctp, "rna_test_full.csv"))
