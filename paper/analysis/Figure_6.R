...
base <- file.path(cfg$out_root, "COVID_PBMC_CITE_seq", "LODO_1_test_NS1")
...
# keep_cells_aligned <- names(groups)
...
#  add cell type here and see does cell types are different from regime
...
p1 <- ggplot(plot_df_te, aes(UMAP_1, UMAP_2, color = regime)) +
  ...
  theme_classic(base_size = 8) +
  labs(title = "Test donor NS1 UMAP: regime (argmax α)", color = "Regime") +        # NS1, TP7 and TS5
  theme(plot.title = element_text(hjust = 0.5,  face = "bold"))
# labs(title = "Test donor TP7 UMAP: \nregime (argmax α)", color = "Regime") +        # NS1, TP7 and TS5
...
p2 <- ggplot(plot_df_te, aes(UMAP_1, UMAP_2, color = entropy)) +
  geom_point(size = 0.25, alpha = 0.7) +
  theme_classic(base_size = 8) +
  labs(title = "Test donor NS1 UMAP: \nα-entropy", color = "Entropy")
...
comp_te_ct <- celltype_regime_comp(alpha_te, seu_prep, cell_ids=test_cells,
                                   celltype_col=ct_col, min_n=min_n, split_label="test donors NS1")
...
p3_left_mono <- ggplot(mono_df, aes(UMAP_1, UMAP_2, color = score_sm)) +
  ...
  theme_classic(base_size = 8) +
  labs(
    title = "Monocytes: IFN-activation axis",
...
pathway_df_mono <- bind_rows(
  hallmark_concordant %>%
    ...
    transmute(
      pathway = case_when(
        ID == "HALLMARK_INTERFERON_GAMMA_RESPONSE" ~ "Hallmark IFN-\u03b3",
...
p3_right_mono <- ggplot(pathway_df_mono, aes(x = NES, y = pathway)) +
  ...
  labs(
    x        = "Test-donor NES1",
    y        = NULL,
    title    = "Reproducible monocyte IFN-activation\nand biosynthetic pathways",
    subtitle = "Concordant in train and test donors;\nbars show test-donor NES1"
  ) +
  theme(
    plot.title   = element_text(face = "bold"),
...
p3_left_4T <- ggplot(cd4_df, aes(UMAP_1, UMAP_2, color = score)) +
  ...
  theme_classic(base_size = 8) +
  labs(
    title = "CD4\u207a T cells: \nactivated versus resting/biosynthetic axis",
    subtitle = "PBMC, test cells",
    x = "UMAP 1",
    y = "UMAP 2"
  ) +
...
p3_right_4T <- ggplot(pathway_df_4T, aes(x = NES, y = pathway)) +
  ...
  labs(
    x        = "Test-donor NES1",
    y        = NULL,
    title    = "Reproducible CD4\u207a T activation \nand biosynthetic pathways",
...
p3_right_8T <- ggplot(pathway_df_8T, aes(x = NES, y = pathway)) +
  ...
  labs(
    x        = "Test-donor NES1",
    y        = NULL,
    title = "Reproducible CD8\u207a T IFN-associated \nand biosynthetic pathways",
    subtitle = "Concordant in train donor and test donor; \nbars show test donor NES1"
  ) +
  theme(
    plot.title   = element_text(face = "bold"),
...
ggsave(file.path(out_dir, "Figure_6.pdf"),
       plot = fig,
       device = cairo_pdf,
