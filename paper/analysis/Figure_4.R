...
cd27_celltypes <- c("B", "CD4 T", "CD8 T", "NK", "other T")
comp_cd27 <- comp_all_ct2 %>%
  dplyr::filter(celltype %in% cd27_celltypes)
...
p_cd27 <- ggplot(comp_cd27, 
  ...
  scale_y_continuous(labels = scales::percent_format(),
                     expand = c(0, 0)) +
  scale_x_discrete(limits = cd27_celltypes) +
  theme_classic(base_size = 8) +
  theme(
    axis.text.x  = element_text(angle = 45, hjust = 1)
  ) +
  labs(
    title = "Cross-dataset CD27 regime composition\nin shared lymphoid cell types",
    x = NULL,
    y = "Fraction"
  )
...
p3_left_8T <- ggplot(mono_df, aes(UMAP_1, UMAP_2, color = score)) +
  ...
  theme_classic(base_size = 8) +
  labs(
    title = "CD8\u207a T cells:cytotoxic NK-like \neffector continuum",
...
pathway_df_8T <- bind_rows(
  hallmark_concordant %>%
    filter(ID == "HALLMARK_MYC_TARGETS_V1") %>%
    transmute(
      pathway = "Hallmark MYC targets",
      NES_tr  = NES_tr,
      NES_te  = NES_te,
      padj_tr = padj_tr,
      padj_te = padj_te,
...
xmin <- min(pathway_df_8T$NES_te, na.rm = TRUE)
xmax <- max(pathway_df_8T$NES_te, na.rm = TRUE)

p3_right_8T <- ggplot(pathway_df_8T, aes(x = NES_te, y = pathway)) +
  geom_col(
    aes(fill = NES_te > 0),
    width = 0.65,
    show.legend = FALSE
  ...
  coord_cartesian(xlim = c(xmin - 0.03, 0.02), clip = "off") +
  theme_classic(base_size = 8) +
  labs(
    title = "CD8\u207a T MYC and biosynthetic \npathway suppression",
...
p3_left_B <- ggplot(mono_df, aes(UMAP_1, UMAP_2, color = score)) +
  ...
  theme_classic(base_size = 8) +
  labs(
    title = "B cells:\nmature\u2013precursor-like CD27 axis",
    subtitle = "Healthy young BM, test cells",
    x = "UMAP 1", y = "UMAP 2"
  )+ theme(
    plot.title      = element_text(face = "bold"),
...
pathway_df_B <- reactome_concordant %>%
  filter(ID %in% c(
    "REACTOME_CELLULAR_RESPONSE_TO_STARVATION",
  ...
  transmute(
    pathway = case_when(
      ID == "REACTOME_CELLULAR_RESPONSE_TO_STARVATION" ~ "Starvation response",
      ID == "REACTOME_EUKARYOTIC_TRANSLATION_ELONGATION" ~ "Translation elongation",
      ID == "REACTOME_NONSENSE_MEDIATED_DECAY_NMD" ~ "NMD",
      ID == "REACTOME_RESPONSE_OF_EIF2AK4_GCN2_TO_AMINO_ACID_DEFICIENCY" ~ "GCN2 amino-acid deficiency response",
      ID == "REACTOME_RRNA_PROCESSING" ~ "rRNA processing"
    ),
    NES_tr  = NES_tr,
    NES_te  = NES_te,
...
xmin <- min(pathway_df_B$NES_te, na.rm = TRUE)
xmax <- max(pathway_df_B$NES_te, na.rm = TRUE)

p3_right_B <- ggplot(pathway_df_B, aes(x = NES_te, y = pathway)) +
  geom_col(
    aes(fill = NES_te > 0),
    width = 0.65,
    show.legend = FALSE
  ...
  theme_classic(base_size = 8) +
  labs(
    title = "Concordant B-cell biosynthetic and\ntranslational pathway activation",
    subtitle = "Test dataset: healthy young BM",
    x        = "Test NES (healthy young BM)",
    y        = NULL
  )+
...
p_cd27          <- p_cd27          + labs(tag="(h)") + tag_theme1
p3_left_8T <- p3_left_8T + labs(tag="(i)") + tag_theme1
p3_right_8T <- p3_right_8T + labs(tag="(j)") + tag_theme1
p3_left_B <- p3_left_B + labs(tag="(k)") + tag_theme1
p3_right_B <- p3_right_B + labs(tag="(l)") + tag_theme1


fig <- 
  (pA | pB | pC | p_ctpnet) /
  (p_sclinear |pD | p1 | p_cd27) /
  (p3_left_8T | p3_right_8T| p3_left_B | p3_right_B) + 
